...
import { TransactionDb } from "../route";
...
export async function POST(req: Request): Promise<NextResponse<ApiResponse<RespSuggestCategories>>> {
    ...
    try {
        const categories = await Promise.all(transactions.map(async (trans) => {
